...
<nav class="fixed-top">
			...
			<a class="usuario" href="controlador?acao=perfilUsuario"><img style="width: 90%; height: 93%;" src="imagens/usuario.png"></a>
				<c:choose>
					<c:when test="${empty online}">
						<div class="cor">
							<a href="controlador?acao=formLogin" class="aa"
								style="font-weight: bold">Entre ou Cadastre-se</a>
						</div>
					</c:when>
					<c:otherwise>
						<div class="cor">
							<a href="controlador?acao=logoutUsuario" class="aa"
	...
	<div class="container">
		...
		<h3 style="text-align: center;"> Preencha os campos abaixo para se cadastrar na PolliEsportiva. </h3><br>
		<div style="margin: auto; width: 75%;">
			<form style="margin: auto; width: 100%;" action="controladorC?acao=cadastrarCliente" method="post" enctype="multipart/form-data">
				<div class="custom-file">
					<label style="color: #495057; font-weight: normal;" class="custom-file-label" for="customFileLang">Imagem do perfil...</label>
					<input aria-describedby="imgCliente" id="customFileLang" name="imagemPerfil" lang="pt-BR" style="background-color:  #FF7804;float: right;" type="file" class="custom-file-input">
				</div>
				<div class="form-group">
					<label for="nomeCliente"> Nome: </label>
					<input required="required" class="form-control" type="text" id="nomeCliente" name="nomeCliente" pattern="[a-zA-Z\s]*" placeholder="Nome...">
				</div>
				<div class="form-group">
					<label for="sexoCliente"> Sexo: </label>
					<div class="form-check">
						<input required class="form-check-input" type="radio" id="sexoClienteF" name="sexoCliente" value="Feminino">
						<label class="form-check-label" for="sexoClienteF">
						  Feminino
						</label>
					  </div>
					  <div class="form-check">
						<input required class="form-check-input" type="radio" id="sexoClienteM" name="sexoCliente" value="Masculino">
						<label class="form-check-label" for="sexoClienteM">
						  Masculino
						</label>
					  </div>
				</div>
				<div class="form-group">
					...
					<input required="required" class="form-control" type="text" id="cpfCliente" name="cpfCliente" placeholder="___.___.___-__" pattern="[0-9]{3}.[0-9]{3}.[0-9]{3}-[0-9]{2}">
				</div>
				<div class="form-group">
					<label for="nascimentoCliente"> Data de Nascimento: </label>
					<input required="required" class="form-control" type="date" id="nascimentoCliente" name="nascimentoCliente">
				</div>
				<div class="form-group">
					<label for="telefoneCliente"> Telefone: </label>
					<input required="required" class="form-control" type="text" id="telefoneCliente" name="telefoneCliente" placeholder="(__) _ ____-____" pattern="\([0-9]{2}\)[\s][0-9]{1}[\s][0-9]{4}-[0-9]{4}">
				</div>
				<div class="form-group">
					<label for="emailCliente"> E-mail: </label>
					<input required="required" class="form-control" type="email" id="emailCliente" name="emailCliente" placeholder="email@exemplo.com">
				</div>
				<div class="form-group">
					<label for="senhaCliente"> Senha: </label>
					<input required="required" class="form-control" type="password" id="senhaCliente" name="senhaCliente" placeholder="Senha...">
				</div>
				<div class="form-group">
					<div class="form-check">
						<input class="form-check-input" type="checkbox" id="desejaNotificacao" name="desejaNotificacao" value="Sim">
						<label class="form-check-label" for="desejaNotificacao">
						  Desejo receber notificações de novas promoções da loja.
						</label>
					</div>
				</div>
				<button id="botCad" style="float: right; background-color: #FF7804; border-color: #CC5F02;" type="submit" class="btn btn-primary mb-2" >Cadastrar <i class="fa fa-plus-circle" aria-hidden="true"></i></button>
			</form>
		</div>
