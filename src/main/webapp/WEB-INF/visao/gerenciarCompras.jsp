...
<div style="margin: auto; display: flex; justify-content:space-between; max-width: 90%; padding-top: 2.5%;padding-bottom: 2.5%;">
	<div class="container" style="max-width: 98%;">
		...
		<h1 style="text-align: center;"> Gerenciamento de Compras </h1><br>
		<br><h4 style="text-align: center;"> ${msg} </h4><br>
			<c:choose> 
				<c:when test="${compras.size() > 0}">
					<table class="table table-hover" style="margin: auto; width: 100%; font-size: 11px">
						...
						<c:forEach var="compra" items="${compras}">
							<tr>
								...
								<td>${compra.avaliacao}</td>
								<td>${compra.comentario}</td>
								<td>${compra.dataHora.time}</td>
								<td style="justify-content:space-between;">
									<a href="controladorCompra?acao=detalhesCompra&cod=${compra.cod}"><button id="botCad" style="margin: auto;background-color: #8F8; border-color:  #CC5F02; color: black; font-weight: bold; height: 100%" class="btn btn-primary mb-2">Detalhes <i class="fa fa-search-plus"></i></button></a>
								</td>
							</tr>
						</c:forEach>
						</tbody>
					</table><br>
					<a href="controlador?acao=perfilUsuario"><button id="botCad" style="float: right; background-color: #FF7804; border-color: #CC5F02; font-weight: bold;" class="btn btn-primary mb-2" > Voltar ao Perfil </button></a>
				</c:when>
				<c:otherwise>
					<h3 style="text-align: center; color: red;">Não há Compras Cadastradas!</h3><br><br>
