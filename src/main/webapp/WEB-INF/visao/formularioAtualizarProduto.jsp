	...
	<div style="margin: auto; width: 75%;">
			<c:choose>
				<c:when test="${Produto != null}">
					<h3 style="text-align: center;"> Atualização do Produto de código "${Produto.cod}":</h3><br>
					<form style="margin: auto; width: 100%;" action="controladorP?acao=atualizarProduto&cod=${Produto.cod}" method="post" enctype="multipart/form-data">
						<div class="custom-file">
							<label style="color: #495057; font-weight: normal; z-index: 10" class="custom-file-label" for="imagemProduto">Imagem do Produto... </label>
							<input aria-describedby="imgProduto" id="imagemProduto" name="imagemProduto" style="float: right; z-index: 10" type="file" class="custom-file-input">
							<input type="hidden" name="naoAtualizado" value="${Produto.nomeArq}">
							<c:if test="${Produto.nomeArq != null}">
								<br><br><br>
								<h6 style="font-weight: bold">Imagem já Cadastrada: 
									<img class="prodImg" title="${Produto.nomeArq}" style="position: relative; left: 19%; max-width: 20%; max-height: 20%; z-index: 100;" alt="*Imagem*" src="${Produto.nomeArq}">
								</h6>
								<br>
							</c:if>
						</div>
						<div class="form-group">
							...
							<input required="required" class="form-control" type="text" id="nomeProduto" name="nomeProduto" value="${Produto.nome}">
						</div>
						<div class="form-group">
							...
							<div class="input-group">
								...
								<input required="required" class="form-control" type="text" id="precoProduto" name="precoProduto" value="${Produto.preco}">
							</div>
						</div>
						<div class="form-group">
							<label  for="marcaProduto"> Marca: </label>
							<input required="required" class="form-control" type="text" id="marcaProduto" name="marcaProduto" value="${Produto.marca}">
						</div>
						<div class="form-group">
							<label  for="fornecedorProduto"> Fornecedor: </label>
							<input required="required" class="form-control" type="text" id="fornecedorProduto" name="fornecedorProduto" value="${Produto.fornecedor}">
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<select required="required" class="form-control" name="subcategoriaProduto" id="subcategoriaProduto"> 
								<option value="${Produto.subcategoria}">${Produto.subcategoria}</option>
						...
						<div class="form-group">
							...
							<textarea style="resize: none;" rows="10" cols="30" required="required" class="form-control" id="descricaoProduto" name="descricaoProduto" maxlength="680">${Produto.descricao}</textarea>
						</div>
						<button style="float: right; background-color: #FF7804; color: black; border-color:  #CC5F02;" id="botCad" type="submit" class="btn btn-primary mb-2" >Atualizar Produto <i class="fa fa-undo" aria-hidden="true"></i></button>
					</form>
				</c:when>
				<c:otherwise>
					<h3 style="text-align: center; color: red;">Este produto não está cadastrado!</h3><br>
					<div style="width: 50%; margin: auto; display: flex; justify-content: center;">
						<a href="controladorP?acao=gerenciarProdutos"><button id="botCad" style="background-color: #FF7804; border-color: #CC5F02; font-weight: bold;" class="btn btn-primary mb-2" >Voltar ao Gerenciamento de Produtos <i class="fa fa-long-arrow-right" aria-hidden="true"></i></button></a>
					</div>
				</c:otherwise>
			</c:choose>
		</div>
